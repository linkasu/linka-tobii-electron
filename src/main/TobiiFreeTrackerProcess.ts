...
export class TobiiFreeTrackerProcess extends EventEmitter implements EyeTrackerProcess {
  private readonly resolveExtraResource: (...segments: string[]) => string;
  ...
  constructor (options: TobiiFreeTrackerProcessOptions = {}) {
    super();
    this.resolveExtraResource = options.resolveExtraResource || resolvePackageExtraResource;
    const socketName = options.socketName || "su.linka.tobii";
    this.socketPath = process.env.TOBIIFREE_SERVICE_SOCKET || join(tmpdir(), `${socketName}.${typeof process.getuid === "function" ? process.getuid() : "user"}.sock`);
