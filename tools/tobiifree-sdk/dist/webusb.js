...
const log = (...args) => console.log('[webusb]', ...args);
const logErr = (...args) => console.error('[webusb]', ...args);
...
export class WebUsbTransport {
    ...
    static async fromDevice(device) {
        ...
        if (r.status !== 'ok')
            throw new Error(`session-open failed: ${r.status}`);
        log('session opened');
    ...
    async recv(signal, onChunk) {
        ...
        while (!signal.aborted) {
            ...
            if (r.status !== 'ok' || !r.data)
                throw new Error(`bulk IN: ${r.status}`);
            chunks++;
