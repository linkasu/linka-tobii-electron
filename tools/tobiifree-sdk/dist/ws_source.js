...
const log = (...args) => console.log('[ws-source]', ...args);
const logErr = (...args) => console.error('[ws-source]', ...args);
// Daemon protocol constants (must match daemon_protocol.zig).
const HEADER_SIZE = 5;
const SRV = { GAZE: 0x01, RESPONSE: 0x02, DISPLAY_AREA: 0x03, ERR: 0xFF };
const CMD = {
    SUBSCRIBE: 0x01,
...
export class WsSource {
    ...
    closed = false;
    _displayArea = null;
    constructor(ws, requestTimeoutMs) {
        this.ws = ws;
    ...
    static async connect(url, requestTimeoutMs = 2000) {
    ...
    async setDisplayAreaCorners(area) {
        this.sendCmd(CMD.SET_DISPLAY_AREA_CORNERS, encodeF64s(area.tl.x, area.tl.y, area.tl.z, area.tr.x, area.tr.y, area.tr.z, area.bl.x, area.bl.y, area.bl.z));
        try {
            await this.getDisplayArea();
        }
        catch { }
    ...
    request(cmdType, payload, timeoutMs) {
    ...
    onBinary(data) {
        ...
        while (pos + HEADER_SIZE <= this.recvBuf.byteLength) {
            ...
            const payloadLen = new DataView(this.recvBuf.buffer, this.recvBuf.byteOffset + pos + 1, 4).getUint32(0, true);
            const msgEnd = pos + HEADER_SIZE + payloadLen;
            if (msgEnd > this.recvBuf.byteLength)
                break;
        ...
        switch (msgType) {
            case SRV.GAZE: {
                if (payload.byteLength >= 232) {
                    ...
                    for (const l of this.gazeListeners)
                        l(sample);
                }
                break;
            }
...
//# sourceMappingURL=ws_source.js.map