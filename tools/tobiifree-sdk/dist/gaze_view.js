...
export function readGazeSample(buffer, ptr) {
    ...
    if (mask & BIT_FRAME_COUNTER)
        s.frame_counter = dv.getUint32(OFF_FRAME_COUNTER, true);
    if (mask & BIT_VALIDITY_L)
        s.validity_L = dv.getUint32(OFF_VALIDITY_L, true);
    if (mask & BIT_VALIDITY_R)
        s.validity_R = dv.getUint32(OFF_VALIDITY_R, true);
    if (mask & BIT_PUPIL_L)
        s.pupil_diameter_L_mm = dv.getFloat64(OFF_PUPIL_L, true);
    if (mask & BIT_PUPIL_R)
        s.pupil_diameter_R_mm = dv.getFloat64(OFF_PUPIL_R, true);
    if (mask & BIT_GAZE_2D) {
    ...
    if (mask & BIT_EYE_ORIGIN_L)
        s.eye_origin_L_mm = readV3(dv, OFF_EYE_ORIGIN_L);
    if (mask & BIT_EYE_ORIGIN_R)
        s.eye_origin_R_mm = readV3(dv, OFF_EYE_ORIGIN_R);
    if (mask & BIT_GAZE_DIR_L)
        s.trackbox_eye_pos_L = readV3(dv, OFF_GAZE_DIR_L);
    if (mask & BIT_GAZE_DIR_R)
        s.trackbox_eye_pos_R = readV3(dv, OFF_GAZE_DIR_R);
    if (mask & BIT_GAZE_3D_L)
        s.gaze_point_3d_L_mm = readV3(dv, OFF_GAZE_3D_L);
    if (mask & BIT_GAZE_3D_R)
        s.gaze_point_3d_R_mm = readV3(dv, OFF_GAZE_3D_R);
    if (mask & BIT_EYE_ORIGIN_L_DISP)
        s.eye_origin_L_display_mm = readV3(dv, OFF_EYE_ORIGIN_L_DISP);
    if (mask & BIT_EYE_ORIGIN_R_DISP)
        s.eye_origin_R_display_mm = readV3(dv, OFF_EYE_ORIGIN_R_DISP);
    if (mask & BIT_TRACKBOX_L_DISP)
        s.trackbox_eye_pos_L_display = readV3(dv, OFF_TRACKBOX_L_DISP);
    if (mask & BIT_TRACKBOX_R_DISP)
        s.trackbox_eye_pos_R_display = readV3(dv, OFF_TRACKBOX_R_DISP);
    if (mask & BIT_EYE_ORIGIN_RAW_L)
        s.eye_origin_raw_L_mm = readV3(dv, OFF_EYE_ORIGIN_RAW_L);
    if (mask & BIT_EYE_ORIGIN_RAW_R)
        s.eye_origin_raw_R_mm = readV3(dv, OFF_EYE_ORIGIN_RAW_R);
...
//# sourceMappingURL=gaze_view.js.map